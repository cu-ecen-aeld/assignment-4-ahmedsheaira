...
void* threadfunc(void* thread_param)
{
    ...
    DEBUG_LOG("Thread started, will wait to obtain mutex for %d ms", thread_func_args->wait_to_obtain_ms);

    usleep(thread_func_args->wait_to_obtain_ms * 1000);

    DEBUG_LOG("Attempting to optain mutex");
    ...
    DEBUG_LOG("Mutex locked, will wait for %d ms", thread_func_args->wait_to_release_ms);
